...
import os
import pandas as pd
from config import CHUNK_SIZE, SMALL_FILE_THRESHOLD, SUPPORTED_EXTENSIONS
...
class FileParsingPipeline:
    ...
    def parse(self) -> pd.DataFrame:
        ...
        # Count rows first to decide strategy (only cheap for CSV)
    ...
    def _estimate_row_count(self) -> int | None:
        """
        Fast row-count estimate for CSV only (line count − 1 for header).
        Returns None for Excel (can't cheaply estimate without reading).
        """
        if self.ext == ".csv":
            with open(self.filepath, "r", encoding="utf-8", errors="replace") as f:
                count = sum(1 for _ in f) - 1  # subtract header line
            return max(count, 0)
        return None  # Excel: decide after direct-load attempt
    ...
    def _chunked_load(self) -> pd.DataFrame:
        """Load file in chunks of CHUNK_SIZE and concatenate."""
        ...
        if self.ext == ".csv":
            ...
            # Excel: openpyxl doesn't support true streaming; load then chunk
            df = self._read_file()
            n = len(df)
    ...
    def _infer_schema(self, df: pd.DataFrame) -> dict:
        """Return a dict of {column_name: inferred_dtype_string}."""
        schema = {}
